 ...
 * 
 * Look up for many methods enabling the plugin
 * to be compatible with later versions of the
 * server.
 */
...
//     }
    ...
    public static String getName(final Entity entity) {
        try {
            ...
            return entity instanceof Player ? ((Player) entity).getName() : SMCommon.beautifyCapitalize(entity.getType());
        }
    }
    ...
    public static <T> T invokeStatic(@NonNull final Method method, final Object... params) {
		try {
			SMValid.checkBoolean(Modifier.isStatic(method.getModifiers()),
					"Method " + method.getName() + " must be static to be invoked through invokeStatic with params: " + SMCommon.join(params));

			return (T) method.invoke(null, params);

		} catch (final ReflectiveOperationException ex) {
			throw new SMBridgeException(ex, "Could not invoke static method " + method + " with params " + SMCommon.join(params, ", "));
		}
	}
}
