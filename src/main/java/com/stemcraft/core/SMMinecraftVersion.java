...
public class SMMinecraftVersion {
	/**
	...
	public enum V {
		...
		protected static V parse(int number) {
			...
			throw new SMException("Invalid version number: " + number);
		}
	...
	static {
		try {
			...
			if (hasGatekeeper) {
				...
				SMValid.checkBoolean(found == 1, "Minecraft Version checker malfunction. Could not detect your server version. Detected: " + numericVersion + " Current: " + curr);
			...
			STEMCraft.error(t, "Error detecting your Minecraft version. Check your server compatibility.");
		}
	}
}
